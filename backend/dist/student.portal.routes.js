...
import { requireAuth, requireStudent, } from "./auth.middleware.js";
...
router.get("/allocation", async (req, res) => {
    try {
        ...
        console.error(error);
        return res.status(500).json({
            error: "Failed to load allocation.",
        });
    }
});
    ...
    try {
        ...
        if (existingAllocation) {
            return res.status(409).json({
                error: "Preferences are locked while an allocation exists. Ask the administrator to reset the allocation.",
            });
        }
        ...
        if (courseIds.length > 5) {
            return res.status(400).json({
                error: "You can select a maximum of 5 courses.",
            });
        }
        const uniqueCourseIds = new Set(courseIds);
        if (uniqueCourseIds.size !==
            courseIds.length) {
            return res.status(400).json({
                error: "A course cannot appear more than once.",
            });
        }
        const courses = await prisma.course.findMany({
            where: {
