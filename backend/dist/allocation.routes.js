...
router.post("/run", async (req, res) => {
    try {
        ...
        const solverInput = {
            ...
            courses: Object.fromEntries(courses.map((course) => [
                course.code,
                course.capacity,
            ])),
            preferences: Object.fromEntries(students.map((student) => {
        ...
        pythonProcess.on("close", async (code) => {
             ...
             */
            if (!solverResult ||
                !Array.isArray(solverResult.allocations)) {
                console.error("Unexpected solver response:", solverResult);
                return res.status(500).json({
                    error: "Solver returned an unexpected response",
                });
            }
            if (solverResult.status === "infeasible") {
                return res.status(400).json({
                    error: "Allocation is infeasible.",
                    details: "One or more students cannot be allocated because they have no valid course preferences.",
                    solver: solverResult,
                });
            }
            /*
             ...
             * ------------------------------------------------
             */
            const allocationRows = solverResult.allocations
                .map((allocation) => {
                const student = students.find((student) => student.name ===
                    allocation.student);
                const course = courses.find((course) => course.code ===
                    allocation.course);
                if (!student ||
                    !course) {
                    return null;
             ...
             * ------------------------------------------------
             */
            const savedAllocations = await prisma.allocation.findMany({
                include: {
                    student: true,
                    course: true,
                },
        ...
        console.error("Allocation route failed:", error);
        return res.status(500).json({
